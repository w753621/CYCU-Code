...
class Parser {
  ...
  void TranslateToken() {      // 把輸入的sexp轉成dotted pair的形式。
    ...
    if ( mAccurate_token_vector.size() == 1 ) {
      ...
      while ( mAccurate_token_vector.size() > anumber ) {
        if ( mAccurate_token_vector[anumber].token_type == "LEFT-PAREN" ) {    // 先記錄該子或父的sexp當中有幾個空白先
          ...
          if ( mAccurate_token_vector[anumber-2].token_name == "(" ) {    
          ...
          if ( mAccurate_token_vector[anumber-1].token_type != "DOT" ) {
            connectnil = true;
            mTranslated_token_vector.push_back( token_Dot_data );
          ...
          anumber++;  
          // 製作.nil，在最後後面接nil。
          if ( ( mAccurate_token_vector[anumber].token_type == "RIGHT-PAREN" && connectnil ) 
               || anumber == mAccurate_token_vector.size() ) {   
            mTranslated_token_vector.push_back( token_Dot_data );
            mTranslated_token_vector.push_back( token_NIL_data );     
          } // if             
    ...
    // mTranslated_token_vector 右誇號數量不對等情況，陷入無窮迴圈)。
    int i = 0;
    stack<AQuoteCheck> quote;
    while ( mTranslated_token_vector.size() > i && !aQuoteAtom ) {
      if ( mTranslated_token_vector[i].token_type == "QUOTE" ) {
        AQuoteCheck check;
        check.start = 1;  
        quote.push( check );
      } // if
      else if ( mTranslated_token_vector[i].token_type == "LEFT-PAREN" && quote.size() > 0 ) {
        AQuoteCheck check;
        check = quote.top();
        check.start++;
        quote.pop();
      ...
      else if ( mTranslated_token_vector[i].token_type == "RIGHT-PAREN" && quote.size() > 0 ) {
        AQuoteCheck check;
        check = quote.top();
        check.start--;
        if ( check.start == 0 ) {
  ...
  TreeNode * BuildTree( TreeNode * aTreeRoot, int & i, int & lpnumber ) {
    TreeNode * newnode;
    if ( mTranslated_token_vector[i].token_name == "." || mTranslated_token_vector[i].token_name == ")" ) {
      if ( mTranslated_token_vector[i].token_name == "." ) {
        i++;
        return NULL;
      } // if
      else {  
        aTreeRoot -> isEnd = true;
        bool bottom = false;
      ...
      if ( ( newnode -> token_data.token_name == "(" && lpnumber == i ) ||  
           ( newnode -> token_data.token_name == "("  && mTranslated_token_vector[i].token_name == "(" ) 
           || IsQuoteStart( i, newnode ) ) {     
      ...
      else if ( mTranslated_token_vector[i].token_name == "." ) {    // 忽略DOT
        i++;
      } // else if  
  ...
  void PrintVector() {
    int i = 0;
    while ( mAccurate_token_vector.size() > i ) {
      cout << mAccurate_token_vector[i].token_type << "\n";
      i++;
    } // while 
...
class Tree {
  ...
  void PrintSExp( TreeNode * aTreeRoot, string & printed, bool & firstsexp ) { 
    if ( aTreeRoot ) {    
      ...
      else if ( ( aTreeRoot -> right == NULL && aTreeRoot -> left == NULL ) 
                || aTreeRoot -> token_data.token_type == "QUOTE" ) {
        if ( firstsexp ) {
          firstsexp = false;
        } // if
        else {
...
TreeNode * ReadSExp( TreeNode * aTreeRoot, int & lastspace ) {  
  ...
  aParser.PrintVector();
  aTreeRoot = aParser.BuildTree( aTreeRoot, i, lpnumber );
  return aTreeRoot;
} // ReadSExp()
...
int main() {
  ...
  while ( !finished ) {                                  
    ...
    try {
      aTreeRoot = ReadSExp( aTreeRoot, lastspace );  // Scanner與Paser互相溝通的function，等Paser處理完並無錯之後就開始建Tree。
      if ( aTreeRoot -> token_data.token_name == "(" && 
           aTreeRoot -> left -> token_data.token_name == "exit" && 
           aTreeRoot -> right -> token_data.token_name == "nil"  ) {   // user讀到 (exit)，結束這次測試檔。

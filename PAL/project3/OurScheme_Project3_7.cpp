...
struct LambdaNameLessInfo { // new出來之後的lambda要找原本的function之表用。
  ...
  TreeNode * point_to_definetable_value;
}; // LambdaNameLessInfo
...
class Tree {
  ...
  TreeNode * CreateProcedureNode( string system_name ) {
    ...
    if ( what_fun_type != -1
      ...
      aLambdaNameLessInfo.point_to_definetable_value = gDefineTable[what_fun_type].define_value;
        ...
        if ( fun_name != "lambda" ) {
          ...
          if ( function_pos == -1 ) {
            ...
            while ( mLambdaNameLessInfo.size() > i ) {
              if ( fun_node == mLambdaNameLessInfo[i].point_to_orgin_node ) {
                TreeNode * temp_node = mLambdaNameLessInfo[i].point_to_definetable_value;
                function_pos = FindNameless_UserFunctionPos( temp_node );
                fun_name = gDefineTable[function_pos].user_define_name;

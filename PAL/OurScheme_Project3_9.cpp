...
struct LambdaNameLessInfo { // new出來之後的lambda要找原本的function之表用。
  TreeNode * point_to_orgin_node;
  string define_table_name;
}; // LambdaNameLessInfo

vector<DefineTable> gDefineTable;
vector<SystemSymbol> gSystemSymbol;
...
class Tree {
  ...
  vector<LambdaNameLessInfo> mLambdaNameLessInfo;
    ...
    if ( mUserFunctionInfo.size() > 0 && para_node -> token_data.token_type == "SYMBOL" ) {
      ...
      if ( IsUserFunctionParameter( para_node -> token_data, aDefineTable, info_pos ) ) {
        ...
        if ( IsSystemSymbol( name, type ) ) {
          ...
          para_node = CreateProcedureNode( para_node -> token_data.token_name );
        } // if
  ...
  TreeNode * EVAlogical( SaveFunctionParameter aSaveFunPara ) {
    ...
    while ( aSaveFunPara.argument_Num > i && unfinished ) {
      ...
      if ( aSaveFunPara.fun_Name == "not" && eva_fun && CheckNodeIsNULL( para_node ) ) {
        UnboundParameterErrorInfo aUnboundParameterErrorInfo;
        aUnboundParameterErrorInfo.unbound_Parameter_node = mAllFunPara[para_pos] -> left;
        throw aUnboundParameterErrorInfo;
      } // if
      else if ( aSaveFunPara.fun_Name == "or" && eva_fun && CheckNodeIsNULL( para_node ) ) {
      ...
      else if ( aSaveFunPara.fun_Name == "and" && eva_fun && CheckNodeIsNULL( para_node ) ) {
        UnboundConditionErrorInfo aUnboundConditionErrorInfo;
        aUnboundConditionErrorInfo.unbound_Condition_node = mAllFunPara[para_pos] -> left;
        throw aUnboundConditionErrorInfo;
      } // else if
      ...
      aSaveFunPara.parameter_Pos.pop();
      i++;
      if ( para_node != NULL && para_node -> token_data.token_type == "NIL" ) {
        alwaysTrue = false;
      } // if
      else {
        alwaysTrue = true;
        ...
        while ( para1_node -> isEnd == false ) {
          ...
          if ( IsPair( is_Pair_node ) ) {
            if ( is_Pair_node -> left -> token_data.token_type == "SYMBOL"
                 ...
                 && !IsSameSymbol_In_Argument( is_Pair_node -> left -> token_data,
      ...
      if ( !IsList( para1_node ) ) {
        ...
        while ( para1_node -> isEnd == false ) {
          ...
          if ( is_Symbol_node -> token_data.token_type == "SYMBOL" ) {
            if ( Not_Same_Symbol( newDefineTable.fun_argument, is_Symbol_node ) ) {
    ...
    if ( aSaveFunPara.argument_Num == 2 ) {
      ...
      if ( para2_node -> token_data.token_type == "PROCEDURE" ) {
        ...
        if ( change_to_function == "lambda" ) {
          ...
          newDefineTable.user_define_name = para1_node -> token_data.token_name;
          // newDefineTable.user_define_type = "Define-Function";
      ...
      if ( newDefineTable.user_define_type == "Parameter" ) {
        if ( repeat_para1_pos == -1 ) {
          ...
          gDefineTable[repeat_para1_pos] = newDefineTable;
          if ( repeat_function_type == "Define-Function" || repeat_function_type == "Lambda-Function" ) {
            int system_pos = -1;
            system_pos = FindSystemSymbol( newDefineTable.user_define_name );
            if ( system_pos != -1 ) mSystemSymbolTable.erase( mSystemSymbolTable.begin() + system_pos );
          } // if
  ...
  void EVAdefine_Type2( SaveFunctionParameter aSaveFunPara ) {
    ...
    if ( aSaveFunPara.argument_Num >= 2 ) {
      ...
      if ( repeat_para1_pos == -1 ) {
        gDefineTable.push_back( newDefineTable );
        int last_pos = gDefineTable.size() - 1;
        gDefineTable[last_pos].define_value = CreateProcedureNode( newDefineTable.user_define_name );
        InsertUserFunctionToSystem( newDefineTable.user_define_name );
      } // if
      else {
        if ( repeat_function_type == "Parameter" ) {
          gDefineTable[repeat_para1_pos] = newDefineTable;
          string name = newDefineTable.user_define_name;
          gDefineTable[repeat_para1_pos].define_value = CreateProcedureNode( name );
          InsertUserFunctionToSystem( name );
        } // if
        else {
          gDefineTable[repeat_para1_pos] = newDefineTable;
          string name = newDefineTable.user_define_name;
  ...
  bool Not_Same_Symbol( vector<TreeNode*> fun_argument, TreeNode * symbol_node ) {
    int i = 0;
    while ( fun_argument.size() > i ) {
      if ( fun_argument[i] -> token_data.token_name == symbol_node -> token_data.token_name ) {
        return false;
      } // if
  ...
  TreeNode * CreateProcedureNode( string system_name ) {
    ...
    if ( what_fun_type != -1
      ...
      string system_name_or = "lambda";
      string token_name = "#<procedure " + system_name_or + ">";
      procedure_error_name -> token_data.token_name = token_name;
      procedure_error_name -> token_data.token_type = "PROCEDURE";
      LambdaNameLessInfo aLambdaNameLessInfo;
      aLambdaNameLessInfo.point_to_orgin_node = procedure_error_name;
      aLambdaNameLessInfo.define_table_name = system_name;
      mLambdaNameLessInfo.push_back( aLambdaNameLessInfo );
    } // if
    else {
      ...
      if ( mLevel_num == 1 ) {
        if ( argument_num == 0 ) { // 實作 clean-environment
          ...
          if ( fun_name == "exit" ) {
            ...
            gDefineTable.clear();
            newTable.swap( gDefineTable );
            mSystemSymbolTable.clear();
            vector<SystemSymbol> anewtable;
            anewtable.swap( mSystemSymbolTable );
            mSystemSymbolTable = gSystemSymbol;
      ...
      if ( fun_type == "PROCEDURE" ) {
        ...
        if ( fun_name != "lambda" ) {
          ...
          if ( fun_type == "User-Defined-Function" ) {
            ...
            function_pos = FindNameless_UserFunctionPos( gDefineTable[function_pos].define_value );
          ...
          if ( function_pos == -1 ) {
            int i = 0;
            while ( mLambdaNameLessInfo.size() > i ) {
              if ( mLambdaNameLessInfo[i].point_to_orgin_node != NULL &&
                   fun_node != NULL &&
                   fun_node == mLambdaNameLessInfo[i].point_to_orgin_node ) {
                function_pos = FindName_UserFunctionPos( mLambdaNameLessInfo[i].define_table_name );
                function_pos = FindNameless_UserFunctionPos( gDefineTable[function_pos].define_value );
                fun_name = gDefineTable[function_pos].user_define_name;
  ...
  bool IsSameSymbol_In_Argument( TokenData tokendata, vector<TreeNode*> fun_argument, int & repeat_pos ) {
    int i = 0;
    while ( fun_argument.size() > i ) {
      if ( fun_argument[i] != NULL &&
           fun_argument[i] -> token_data.token_name == tokendata.token_name ) {
        repeat_pos = i;
        return true;
      } // if
...
int main() {
  ...
  gSystemSymbol = systemSymbolTable;
  while ( !finished ) {
    ...
    try {
      ...
      if ( !finished ) {
        ...
        TreeNode * resultSExp = NULL;            // 一次處理一個SExp，一個SExp代表一個樹的結構。
        resultSExp = aTree.EvaluateSExp( inputSExp, finished, systemSymbolTable );
        bool firstsexp = true;
        string printed = "";
        // aTree.Preorder( resultSExp );
        if ( aTree.mPrint_SExp && resultSExp ) aTree.PrintSExp( resultSExp, printed, firstsexp );
      } // if

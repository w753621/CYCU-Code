...
vector<CutNumber> DealCutNumber( vector<int> read_number, int cut_number ) {
  ...
  while ( cut_number > i ) {
    if ( i + 1 == cut_number ) {
      ...
      cout << end_pos << "rrrrrrrrr\n";
      cout << start_pos << "eeeeeeeeee\n";
    } // if
    else if ( all_num < all_cut ) {
      if ( one_true == false ) {
        unit = all_cut - all_num;
...
void DealThread( int command, string output_str, fstream & input_file ) {
  ...
  if ( command == 1 ) {
    ...
    i = 0;
    vector<int> merge_sorted_number;
    while ( combine_number[0].aCut_number.size() > i ) {    // 把第一組加入merge中，跟下一組combine做merge。
      merge_sorted_number.push_back( combine_number[0].aCut_number[i] );
      i++;
    } // while

    i = 1;
    std::vector<std::thread> thread_Merge;
    while( combine_number.size() > i ) {                    // 之後merge_number中，跟下一組combine
      thread_Merge.push_back( std::thread( MergeSort, std::ref( merge_sorted_number ), combine_number[i].aCut_number ) );
      cout << i << " ";
      i++;
    } // while

    for( i = 0; i < thread_Merge.size(); i++ ) {
	    if ( thread_Merge[i].joinable() ) {
		  thread_Merge[i].join();
		  cout << i << "\n";
		} // if
    } // for
    ...
    double cpu_used_time = (double)( end - start ) / CLOCKS_PER_SEC; // 出來單位為秒
    DealResult( merge_sorted_number, output_file );
    output_file << "\n" << "CPU執行之時間: " << cpu_used_time << "\n";
  } // else if
    ...
    i = 1;
    while( combine_number.size() > i ) {                    // 之後merge_number中，跟下一組combine
      MergeSort( merge_sorted_number, combine_number[i].aCut_number );
      i++;
    } // while
